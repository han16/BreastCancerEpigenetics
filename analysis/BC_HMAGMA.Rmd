...
* the original code only used the first anchor of Hi-C loops, **why don't use the second anchor information or both anchors**  
...
system("sed -e `s/, /\\t/g` < SNP_aggregate_transcript.txt > BRSTCancer.transcript.annot")
...
```{r, message=F, warning=F, eval=F}
txt <- readLines(paste0(path3, "SNP_aggregate_transcript.txt"))
txt <- gsub(", ", "\t", txt)
writeLines(txt, paste0(path3,"BRSTCancer.transcript.annot"))
```
...
```{r, message=F, warning=F}
BRCA_GWAS_eQTL=read.table(paste0(path3, "BRCA_GWAS_eQTL.txt"), header=T, sep = "\t", fill = TRUE, quote = "")

# 1. Is the column numeric?
is.numeric(BRCA_GWAS_eQTL$p.value)

# 2. Are there any NAs, NaNs, or Infs?
sum(is.na(BRCA_GWAS_eQTL$p.value))
sum(is.nan(BRCA_GWAS_eQTL$p.value))
sum(is.infinite(BRCA_GWAS_eQTL$p.value))

# 3. Are all p-values between 0 and 1?
range(BRCA_GWAS_eQTL$p.value, na.rm = TRUE)
...
ensembl37 <- useEnsembl(biomart = "ENSEMBL_MART_SNP",
                        dataset = "hsapiens_snp",
                        GRCh = 37)

ensembl38 <- useEnsembl(biomart = "ENSEMBL_MART_SNP",
                        dataset = "hsapiens_snp",
                        GRCh = 38)

snps <- c("rs2910686", "rs2548224", "rs4360063", "rs1363974", "rs3797796", "rs7731592")

pos37 <- getBM(attributes = c("refsnp_id", "chr_name", "chrom_start"),
               filters = "snp_filter", values = snps, mart = ensembl37)
pos38 <- getBM(attributes = c("refsnp_id", "chr_name", "chrom_start"),
               filters = "snp_filter", values = snps, mart = ensembl38)
