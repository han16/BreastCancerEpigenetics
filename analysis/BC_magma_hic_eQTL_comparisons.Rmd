...
sig_list <- list(
  ...
  `EP+chrmt loops`  = comparison_table %>% filter(P_HiC < 0.05) %>% pull(GENE),
  `EP+eQTL` = comparison_table %>% filter(P_eQTL < 0.05) %>% pull(GENE),
  `EP+Chrmt loops+eQTL` = comparison_table %>% filter(P_Full < 0.05) %>% pull(GENE)
)
...
ggvenn(
  ...
  stroke_size = 0.5, 
  set_name_size = 0,      # Set to 0 to hide the category titles
  #set_name_size = 5,
  text_size = 4,
